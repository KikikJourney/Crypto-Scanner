...
class ScalpingIntelligenceTests(unittest.TestCase):
    ...
    def test_long_execution_uses_local_structure_not_24h_extreme(self):
        prices15 = [100 + i * 0.02 for i in range(194)]
        prices5 = [103.5 + i * 0.005 for i in range(194)]
        # Keep the 24h extreme deliberately far away. The execution stop
        # must come from recent 5m/15m structure instead.
        plan = build_plan(
            "LONG", prices_to_rows(prices15, 120), prices_to_rows(prices5, 180),
            88,
            {"extreme_low_24": 50, "extreme_high_24": 110, "atr": 1.0},
        )
        self.assertNotEqual(plan["stop"], 49.75)
        self.assertLess(plan["risk_pct"], 2.0)
        self.assertEqual(plan["reward_r"], 2.0)

    def test_max_stop_distance_returns_wait(self):
        prices15 = [100 + i * 0.02 for i in range(194)]
        prices5 = [103.5 + i * 0.005 for i in range(194)]
        # Recent structure is intentionally far enough away to exceed 2%.
