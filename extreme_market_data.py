...
def build_features(rows,current_price):
    ...
    p24=clamp((current_price-lo24)/(hi24-lo24)) if hi24>lo24 else .5
    p48=clamp((current_price-lo48)/(hi48-lo48)) if hi48>lo48 else .5
    atr=_atr(rows); dist_low=(current_price-lo24)/atr if atr else 99; dist_high=(hi24-current_price)/atr if atr else 99
    c=[_close(x) for x in rows]
    move_low=max(0,(c[-49]-current_price)/atr) if atr and len(c)>=49 else 0
    move_high=max(0,(current_price-c[-49])/atr) if atr and len(c)>=49 else 0
